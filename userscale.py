...
dataset = 'm32'
DATA_PATH = f'data_userscale.pkl'
...
def main():
    print(f"--- USER SCALE BIAS EXPERIMENT ---")
    ...
    experiments = [
        ("EXP-13", "Raw Ratings", "IPW", 'Y_raw', run_ipw),
        ("EXP-14", "Raw Ratings", "PSM", 'Y_raw', run_psm),
        ("EXP-15", "Normalized", "IPW", 'Y_norm', run_ipw),
        ("EXP-16", "Normalized", "PSM", 'Y_norm', run_psm),
    ]
    ...
    pd.DataFrame(results).to_csv('userscale_bias_results.csv', index=False)
    print("\nSaved to userscale_bias_results.csv")
